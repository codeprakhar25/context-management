...
\toprule
\textbf{Input note}\\[1pt]
\texttt{\footnotesize ``fixed the retry backoff in the ingest}\\
\texttt{\footnotesize worker --- it was hammering the API''}\\[4pt]
\textbf{Existing tree} (excerpt)\\[1pt]
\texttt{\footnotesize work/slm-lab/\{research, infra\}}\\
\texttt{\footnotesize work/openclaw/\{harness, notes\}}\\
\texttt{\footnotesize inbox/misc}\\[4pt]
\textbf{Gold folder} \texttt{\footnotesize work/slm-lab/infra}\\
\textbf{Occupancy} 7 training notes already filed there,\\
so this note scores in the $3$--$9$ stratum.\\
\midrule
\textbf{What each method does with it}\\[2pt]
\textbf{flat} \hfill puts all 365 folder paths in one\\
\hfill prompt and asks gpt-4o to choose\\[2pt]
\textbf{BM25 / $k$NN} \hfill finds the 5 most similar\\
\hfill \emph{training notes}, returns the majority\\
\hfill folder of those 5; never proposes a\\
\hfill folder with no training notes\\[2pt]
\textbf{cascade} \hfill shortlists 20 folders by nearest\\
\hfill training note, then asks gpt-4o to\\
\hfill pick one of the 20\\[2pt]
\textbf{LoRA} \hfill Llama-3.1-8B fine-tuned on this\\
\hfill tree's own training notes, full\\
\hfill folder list at inference\\
\bottomrule
\end{tabular}
...
Two annotators independently judged 100 notes sampled from corpus~\corpA{}, stratified to match the occupancy buckets, and were shown only the note text and the recorded folder.
They never saw model output.
The question was: is this a sensible home for this note?
...
Counting ambiguous as a confirmed label, $82$--$94\%$ of sampled gold paths are sensible.
...
A cross-vault leakage check found 7 of 2{,}266 validation items ($0.31\%$) with a near-duplicate training note in a different vault, all traced to one plugin's auto-generated placeholder file rather than user-written notes.
...
Thanks to Rocky for an independent annotation pass (\S\ref{sec:annotation}).

\section*{Reproducibility}

Code, corpus manifests with pinned commit SHAs, and the complete run record, including every correction and negative result reached along the way, are public at \url{https://github.com/codeprakhar25/context-management}.
All results in this paper correspond to commit \texttt{6c375c0}.
Third-party note text is not redistributed; the manifests and build scripts reconstruct corpora~\corpB{} and \corpAp{} from their pinned commits.
The occupancy tables and the sensitivity analysis of \S\ref{sec:sensitivity} rebuild from stored per-item run artifacts with no API calls, via \texttt{scripts/occupancy\_sensitivity.py} and \texttt{scripts/eval\_lexical\_baselines.py}.
