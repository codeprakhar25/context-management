...
Code, corpus manifests with pinned commit SHAs, and the complete run record, including every
correction and negative result reached along the way, will be released publicly on acceptance;
they are available to reviewers on request through the programme chairs.
Third-party note text is not redistributed; the manifests and build scripts reconstruct
corpora~\corpB{} and \corpAp{} from their pinned commits.
The occupancy tables and the sensitivity analysis of \S\ref{sec:sensitivity} rebuild from stored
per-item run artifacts with no API calls.
